...
export class PreimageProofGenerator {
  ...
  async generateProof(
    ...
    if (!fs.existsSync(this.circuitWasmPath)) {
      throw new Error(
        `Circuit WASM file not found: ${this.circuitWasmPath}. Please compile the circuit first.`
      );
    }

    if (!fs.existsSync(this.circuitZkeyPath)) {
      throw new Error(
        `Circuit zkey file not found: ${this.circuitZkeyPath}. Please run trusted setup first.`
      );
    }
  ...
  async verifyProof(proof: any, publicSignals: string[]): Promise<boolean> {
    ...
    if (!fs.existsSync(vkeyPath)) {
      throw new Error(
        `Verification key not found: ${vkeyPath}. Please export verification key first.`
      );
    }
