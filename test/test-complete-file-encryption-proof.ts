...
async function testCompleteFileEncryptionProof() {
  ...
  try {
    ...
    const testData =
      'This is a test Agent Metadata JSON file that needs to be encrypted and proven in ZK.';
    ...
    for (const size of fileSizes) {
      ...
      try {
        ...
        console.log(
          `   ${size.toString().padStart(4)} bytes: ${perfTime.toString().padStart(4)}ms ✅`
        );
      } catch (error: any) {
    ...
    const compatibilityTests = [
      ...
      {
        name: 'Binary Data',
        data: Buffer.from([0x01, 0x02, 0x03, 0xab, 0xcd, 0xef]).toString('binary'),
      },
    ];
    ...
    const sealedKeysDiffer =
      JSON.stringify(result1.expectedSealedKey) !== JSON.stringify(result2.expectedSealedKey);

    console.log(`🔐 Security validation:`);
    console.log(
      `   Different nonces produce different commitments: ${commitmentsDiffer ? 'PASS ✅' : 'FAIL ❌'}`
    );
    console.log(
      `   Different public keys produce different sealed keys: ${sealedKeysDiffer ? 'PASS ✅' : 'FAIL ❌'}`
    );
    ...
    console.log(
      `   node build/file_encryption_proof_js/generate_witness.js build/file_encryption_proof_js/file_encryption_proof.wasm ${smallFileInputPath} build/witness.wtns`
    );
    console.log('');
    console.log('3. Generate trusted setup (if needed):');
    console.log('   snarkjs powersoftau new bn128 14 pot14_0000.ptau -v');
    console.log(
      '   snarkjs powersoftau contribute pot14_0000.ptau pot14_0001.ptau --name="Test contribution" -v'
    );
    console.log('   snarkjs powersoftau prepare phase2 pot14_0001.ptau pot14_final.ptau -v');
    console.log(
      '   snarkjs groth16 setup build/file_encryption_proof.r1cs pot14_final.ptau file_encryption_proof.zkey'
    );
    console.log('');
    console.log('4. Generate proof:');
    console.log(
      '   snarkjs groth16 prove file_encryption_proof.zkey build/witness.wtns proof.json public.json'
    );
    ...
    if (successfulTests.length > 0) {
      ...
      console.log(
        `   Supported file sizes: up to ${Math.max(...successfulTests.map(r => r.size))} bytes`
      );
    }
...
// 运行测试
testCompleteFileEncryptionProof()
  .then(() => {
