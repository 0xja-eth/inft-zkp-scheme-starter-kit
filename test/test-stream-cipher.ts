...
async function testStreamCipher() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      ...
      // Show original data
      console.log(
        `   📝 Original data: "${testCase.data.substring(0, 100)}${testCase.data.length > 100 ? '...' : ''}"`
      );
      ...
      console.log(
        `      - Cipher (${cipher.length} bytes): ${cipher.toString('hex').substring(0, 64)}...`
      );
      ...
      const decrypted = await service.decrypt(encrypted, key);
      console.log(
        `   📄 Decrypted data: "${decrypted.substring(0, 100)}${decrypted.length > 100 ? '...' : ''}"`
      );
      console.log(
        `   🔍 Data length - Original: ${testCase.data.length}, Decrypted: ${decrypted.length}`
      );
      ...
      if (testCase.data.length <= 128 * 16) {
