  ...
  async testBasicSealingAndUnsealing() {
    ...
    try {
      ...
      this.logTest(
        ...
        true,
        `Successfully handled ${encryptionKey.length}-byte key`,
        timing
      );
    } catch (error) {
      const timing = Date.now() - startTime;
  ...
  async testDifferentPublicKeyFormats() {
    ...
    for (const test of formatTests) {
      ...
      try {
        ...
        const timing = Date.now() - startTime;
        this.logTest(
          `Public Key Format: ${test.name}`,
          true,
          `Format: ${test.key.substring(0, 20)}...`,
          timing
        );
      } catch (error) {
        const timing = Date.now() - startTime;
    ...
    try {
      ...
      try {
        ...
        this.logTest(
          ...
          true,
          `Parsed Ethereum public key format`,
          timing
        );
      } catch (error) {
        ...
        if (errorMessage.includes('Failed to decrypt')) {
          ...
          this.logTest(
            'Ethereum Wallet Compatibility',
            true,
            'Correctly handled non-X25519 key format',
            timing
          );
        } else {
          throw error;
  ...
  async testPrivateKeyFormats() {
    ...
    for (const test of privateKeyFormats) {
      ...
      try {
        ...
        const timing = Date.now() - startTime;
        this.logTest(
          `Private Key Format: ${test.name}`,
          true,
          `Successfully unsealed with ${test.name} format`,
          timing
        );
      } catch (error) {
        const timing = Date.now() - startTime;
  ...
  async testCorruptedSealedData() {
    ...
    try {
      ...
      try {
        ...
        const errorMessage = (error as Error).message;
        if (
          errorMessage.includes('unsealing failed') ||
